r"""Load NMB documents, resolved facts, conflicts, checks and cited risks, and open a
draft research run.
...
RESOLVED = Path("data/processed/nmb/resolved.json")
...
APPROVAL_RE = re.compile(
    r"authori[sz]ed for issue by the Board of Directors.{0,200}?"
    r"(\d{1,2})(?:st|nd|rd|th)?\s+(January|February|March|April|May|June)\s+(\d{4})", re.I)


def page_text(year: int, page: int) -> str:
    import pypdfium2 as pdfium

    doc = pdfium.PdfDocument(str(RAW / f"NMB_Annual_Report_{year}.pdf"))
    text = doc[page - 1].get_textpage().get_text_range()
    text = text.replace("￾", "-").replace("’", "'")
    return re.sub(r"\s+", " ", text)


def publication_date(year: int, statement_pages: list[int]) -> tuple[date | None, str | None]:
    """Date the board approved the financial statements, read from the statement pages."""
    for p in statement_pages:
        m = APPROVAL_RE.search(page_text(year, p))
...
def main() -> int:
    ...
    with SessionLocal() as s:
        ...
        for year, m in sorted(manifest.items(), key=lambda kv: int(kv[0])):
            ...
            published, evidence = publication_date(y, pages)
            d = SourceDocument(
                security_id=SECURITY, kind="annual_report", fiscal_year=y,
                title=f"NMB Bank Plc Annual Report {year}", publisher="NMB Bank Plc",
                ...
                retrieved_at=datetime.fromisoformat(m["retrieved_at"]), terms_note=TERMS_NOTE)
            s.add(d)
            docs[y] = d
        s.flush()
        ...
        for category, title, year, page, pattern in RISKS:
            m = re.search(pattern, page_text(year, page), re.I)
            if not m:
        ...
        s.merge(DataSourceStatus(source="nmb_annual_reports", last_success_at=now, last_attempt_at=now,
