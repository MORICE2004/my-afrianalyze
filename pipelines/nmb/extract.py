r"""Dual extraction (Camelot + Docling) of NMB statements and key notes.
...
For each report:
  ...
  3. Method B, Docling, reads the page; its tables are assigned to regions by
     their bounding box.
  4. Rows are matched to canonical items (pipelines/nmb/items.py) and the two
     methods are compared cell by cell. Disagreements are returned as
     conflicts; nothing is averaged or picked silently.

Usage:
    .venv\\Scripts\\python -m pipelines.nmb.extract            # all years
    .venv\\Scripts\\python -m pipelines.nmb.extract 2025 2024  # some years
Writes data/processed/nmb/extraction_<year>.json
"""
...
from pipelines.nmb.items import NOTE_SECTIONS, SECTIONS, ItemSpec, Section, normalise_label, parse_number  # noqa: E402

RAW = Path("data/raw/nmb")
OUT = Path("data/processed/nmb")
...
NOTE_REF_RE = re.compile(r"^\d{1,2}(\.\d+)*\s*(\([a-z0-9ivx\-,]+\))*$", re.I)


def _clean_label(text: str) -> str:
    label = normalise_label(text)
    # Drop a trailing note reference such as "10", "11(a)", "24(a-c)", "39 (iii)".
    label = re.sub(r"\s+\d{1,2}(\s*\([a-z0-9ivx\-]+\))*$", "", label)
...
def match(rows: list[Row], sections: list[Section], year: int) -> list[Candidate]:
    ...
    for i, row in enumerate(rows):
        ...
        if sec is None or not row.label:
            continue
        labels = [row.label] + ([row.alt_label] if row.alt_label else [])
        for spec in sec.items:
            role, label = None, None
            for candidate_label in labels:
                if spec.mode == "stage3_of_4":
                    if re.search(spec.pattern.format(year=year), candidate_label):
                        role, label = "current", candidate_label
                    elif re.search(spec.pattern.format(year=year - 1), candidate_label):
                        role, label = "comparative", candidate_label
                elif re.search(spec.pattern.format(year=year), candidate_label):
            ...
            if len(numbers) < 2 and i + 1 < len(rows) and rows[i + 1].section == row.section:
                nxt = rows[i + 1]
                if len(nxt.numbers) >= 2 and len(nxt.label) < 40:
                    numbers, label = nxt.numbers, f"{label} {nxt.label}".strip()
            cur, comp = _pick(spec, numbers)
            if role == "current":
                comp = None
...
def _pick(spec: ItemSpec, numbers: list[Decimal]) -> tuple[Decimal | None, Decimal | None]:
    if spec.mode == "stage3_of_4":
...
def extract_year(year: int, methods: tuple[str, ...] = ("camelot", "docling")) -> dict:
    pdf_path = RAW / f"NMB_Annual_Report_{year}.pdf"
    t0 = time.time()
    with pdfplumber.open(pdf_path) as pdf:
        heights = {i: p.height for i, p in enumerate(pdf.pages, start=1)}
        regions = find_regions(pdf, SECTIONS + NOTE_SECTIONS, candidate_pages(pdf_path, SECTIONS + NOTE_SECTIONS))
    rows: list[Row] = []
    timings = {}
    for m in methods:
        t = time.time()
    ...
    candidates = match(rows, SECTIONS + NOTE_SECTIONS, year)
    result = {
        "fiscal_year": year,
...
def main(argv: list[str]) -> None:
    years = [int(a) for a in argv if a.isdigit()] or [2021, 2022, 2023, 2024, 2025]
    methods = tuple(a.split("=", 1)[1] for a in argv if a.startswith("--method=")) or ("camelot", "docling")
    for y in years:
        res = extract_year(y, methods)
